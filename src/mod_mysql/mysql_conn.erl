...
%%% functions:
%%%     - on data received:
%%%          FieldInfo = mysql_serv:get_result_field_info(MysqlRes)
%%%          AllRows   = mysql_serv:get_result_rows(MysqlRes)
%%%         with FieldInfo = list() of {Table, Field, Length, Name}
%%%          and AllRows = list() of list() representing records
%%%     - on update:
%%%          Affected= mysql_serv:get_result_affected_rows(MysqlRes)
%%%         with Affected = integer()
%%%     - on error:
%%%          Reason    = mysql_serv:get_result_reason(MysqlRes)
%%%         with Reason = string()
%%%-------------------------------------------------------------------
		...
		{ok, Version} ->
			...
			{error, MySQLRes} ->
			    ?Log2(LogFun, error,
				 ...
				 "~p : ~p",
				 [Database,
				  mysql_serv:get_result_reason(MySQLRes)]),
...
%%--------------------------------------------------------------------
loop(#state{recv_pid=RecvPid,log_fun=LogFun}=State) ->
    receive
	{fetch, Queries, From} ->
...
%%     ?Log2(LogFun, debug, "fetch ~p (id ~p)", [Query1,RecvPid]),
	...
	ok ->
	    case get_query_response(LogFun,RecvPid,Version) of
		{updated, Result} ->
			get_last_insert_id(Sock, RecvPid, LogFun, Version, Packet, Result);
		{data, Result} -> {data, Result};
		{error, Result} -> {error, Result}
		end;
	{error, Reason} ->
	    Msg = io_lib:format("Failed sending data on socket : ~p;Sock = ~p;Packet =~p~n", [Reason,Sock,Packet]),
	    {error, Msg}
    end.
%% 在这里获取最后插入ID
get_last_insert_id(Sock, RecvPid, LogFun, Version, Packet, Result) ->
	if  %% 这里 INSERT 得到 SELECT LAST_INSERT_ID();
		Result#mysql_result.affectedrows > 0 ->
			...
			<<3,73,78,83,69,82,84,_/binary>> ->
				...
				case do_send(Sock, Packet2, 0, LogFun) of
				ok ->
					case get_query_response(LogFun, RecvPid, Version) of
					{updated, _Result2} -> {updated, Result};
					{data, Result2} ->								
						LastId = 
							case Result2#mysql_result.rows of
								[[LastIdTemp]] -> LastIdTemp;
								_ -> 0
							end,
						{updated, Result#mysql_result{lastinsertid = LastId}};
					{error, #mysql_result{error = Reason}} ->
						{error, Result#mysql_result{error = Reason}}
					end;
				{error, Reason} ->
...
do_transaction(State, Fun) ->
	case do_query(State, <<"BEGIN">>) of
		{error, _} = Error -> {aborted, Error};
		_ ->
			case catch Fun() of
				error = Error -> rollback(State, Error);
				{error, _} = Error -> rollback(State, Error);
				{'EXIT', _} = Error -> rollback(State, Error);
				Res ->
					case do_query(State, <<"COMMIT">>) of
						{error, _} = Error ->
							rollback(State, {commit_error, Error});
						_ ->
							case Res of
								{atomic, _} -> Res;
								_ -> {atomic, Res}
...
rollback(State, Error) ->
    ...
    {aborted, {Error, {rollback_result, Res}}}.
...
do_execute(State, Name, Params, ExpectedVersion) ->
	Res = case gb_trees:lookup(Name, State#state.prepares) of
			  ...
			  {value, Version} -> mysql_serv:get_prepared(Name, Version);
			  none -> mysql_serv:get_prepared(Name)
		  end,
	case Res of
...
make_statements_for_execute(Name, Params) ->
    ...
    Stmts = lists:foldl(
	      fun({Num, Val}, Acc) ->
		      NumBin = mysql_serv:encode(Num, true),
		      ValBin = mysql_serv:encode(Val, true),
