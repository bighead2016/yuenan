...
-define(F(AccId),             lists:concat([p, AccId, ".log"])). % 文件名
...
-record(state, {state=?OFF, socket=0, heart_beat_pid=0, recv_pid=0, 
                ...
                app_cipher="", resource_cipher="", sn=1}).
...
do(Cmd)          -> ?DO(Cmd).
do(Cmd, Param)   -> ?DO({doing, #arg{cmd=Cmd, param=Param}}).
test(Cmd, Param) -> ?DO({test,  #arg{cmd=Cmd, param=Param}}).
do_error(Reason) -> ?DO({error, Reason}).

show() -> ?EVENT(show).
show(Args) -> ?EVENT({show, Args}).
stop() -> ?EVENT(stop).
...
do_parse_packet(StateData, Pid, Mark) when is_record(StateData, state) ->
    ...
    if
        0 =/= Socket ->
            do_parse_packet_2(Socket, Pid, Mark);
        true ->
            ok
    end.
do_parse_packet_2(Socket, Pid, Mark) ->
        ...
        {inet_async, Socket, Ref, {ok, <<Len:16, Cmd:16, IsZiped:1, _:7>>}} ->          
            ...
            case RecvData of
                ...
                {fail, {inet_async, Socket, _Ref, {error,timeout}}} ->
                    ...
                    do_parse_packet_2(Socket, Pid, Mark); % 10006居然不返回
                {fail, Y} ->
                    ?LOG("~ts - do_parse_packet_1(~p)~n", [<<"收包失败">>, {Socket, Pid, Y}]),  
                    gen_tcp:close(Socket),
                    do_error("socket_error_1")
            end;
        %%超时处理
