%%% 战报
-module(copy_single_report_serv).
...
%% --------------------------------------------------------------------
%% External exports
-export([start_link/2, update_report_idx_cast/5]).
...
do_cast({update_report_idx, CopyId, UserId, Pro, ReportId, EtsReport}, #state{center_node = CenterNode} = State) ->
	gen_server:cast({?MODULE, CenterNode}, {update_report_center, CopyId, UserId, Pro, ReportId, EtsReport}),
	copy_single_report_api:ets_insert_report(EtsReport),
    copy_single_report_api:update_report_idx_cb(CopyId, UserId, Pro, ReportId),
    {?noreply, State};
do_cast({update_report_center, CopyId, UserId, Pro, ReportId, EtsReport}, State) ->
	copy_single_report_api:ets_insert_report(EtsReport),
	copy_single_report_api:update_report_idx_cb(CopyId, UserId, Pro, ReportId),
	{?noreply, State};
do_cast(Msg, State) ->
...
do_info({get_data, rand, FromNode}, State) ->
	List = copy_single_report_api:ets_report_list(),
	IndexList = copy_single_report_api:ets_report_idx_list(),
...
do_info({sync_data, List, IndexList}, State) ->
	...
	copy_single_report_api:ets_insert_report(List),
	ets:delete_all_objects(?CONST_ETS_COPY_SINGLE_REPORT_IDX),
	copy_single_report_api:ets_insert_report_idx(IndexList),
	erlang:garbage_collect(),
	{?noreply, State};
do_info(Info, State) ->
...
update_report_idx_cast(CopyId, UserId, Pro, ReportId, EtsReport) ->
    gen_server:cast(?MODULE, {update_report_idx, CopyId, UserId, Pro, ReportId, EtsReport}).
