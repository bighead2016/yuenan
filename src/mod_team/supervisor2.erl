...
-export([start_link/2, start_link/3,
		 start_child/2, start_child_for_team/2, recycle_team_id/2, restart_child/2,
...
-export([try_again_restart/2, behaviour_info/1]).
...
start_child_for_team(Supervisor, ChildSpec) ->
	call(Supervisor, {start_child_for_team, ChildSpec}).
recycle_team_id(TeamSup, TeamId) ->
	cast(TeamSup, {recycle_team_id, TeamId}).
...
do_select_id(State) ->
	case State#state.id_pool of
        [Id|Pool] -> {ok, State#state{id_pool = Pool}, Id};
        [] -> {error, undefined}
    end.
%%% ---------------------------------------------------
%%% 
...
handle_call({start_child_for_team, EArgs}, _From, State) when ?is_simple(State) ->
    Child		= hd(State#state.children),
    #child{mfargs = {M, F, A}} = Child,
    ArgsTemp	= A ++ EArgs,
	case do_select_id(State) of
		{ok, State2, Id} ->
			Args		= [Id|ArgsTemp],
			Child2		= Child#child{mfargs = {M, F, Args}},
			case do_start_child_i(M, F, Args) of
				{ok, undefined} when Child2#child.restart_type =:= temporary ->
					{reply, {ok, undefined}, State};
				{ok, Pid} ->
					NState = save_dynamic_child(Child2#child.restart_type, Pid, Args, State2),
					{reply, {ok, Pid}, NState};
				{ok, Pid, Extra} ->
					NState = save_dynamic_child(Child2#child.restart_type, Pid, Args, State2),
					{reply, {ok, Pid, Extra}, NState};
...
handle_cast({recycle_team_id, TeamId}, #state{id_pool = IpPool}=State)
  when ?is_simple(State) ->
	State2	= State#state{id_pool = [TeamId|IpPool]},
	{noreply, State2};
...
init_state1(SupName, {Strategy, MaxIntensity, Period}, Mod, Args) ->
    ...
    validIntensity(MaxIntensity),
    validPeriod(Period),
	IdPool = lists:seq(?CONST_TEAM_MIN_ROOM_ID, ?CONST_TEAM_MAX_ROOM_ID),
