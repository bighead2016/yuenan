...
%% External functions
%% ====================================================================
%% manage_net_sup:start_link(gateway_sup, 1).
...
%%     Port = 11111,
	case gen_tcp:listen(Port, ?CONST_TCP_OPTIONS_LISTEN) of
        {?ok, ListenSocket} ->
			...
			ChildSpec      	= misc_app:child_spec(manage_net_worker_sup, manage_net_worker_sup, [],
												  permanent, brutal_kill, supervisor, Cores),
			ChildSpecList 	= misc_app:child_spec_list(CoreList, [], manage_acc_serv, Cores,
													   [ListenSocket], transient, brutal_kill, worker),
			List			= [ChildSpec|ChildSpecList],
			{?ok, {{one_for_one, 100, 5}, List}};
		Reason ->
			?MSG_SYS(" Reason:~p ",[Reason]),
			{?error, Reason}
	end.
...
stop_acceptors() ->
    ...
    F = fun({_, ChildPid, _, ['manage_acc_serv']}) ->
                manage_acc_serv:stop(ChildPid);
           (_) ->
                ?ok
        end,
