...
init([Cores, N, ListenSocket]) ->
    ...
    %% 随机数种子
    ?RANDOM_SEED,
    State   = #state{socket = ListenSocket, cores = Cores, times = N, switch = ?true},
...
handle_info({inet_async, ListenSocket, Ref, {?ok, Socket}},
    ...
    case Switch of
        ?true ->
            case manage_net_worker_sup:start_child_gateway_worker_serv(Cores, Times, Socket, ListenSocket) of
                {?ok, Pid} ->
