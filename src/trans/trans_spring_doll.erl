%% author:	xjg
%% create:	2014-1-10
%% desc:	trans spring_doll record in the database
%%

-module(trans_spring_doll).
...
-export([trans_spring_doll/0]).

trans_spring_doll() ->
	misc_sys:init(),
	mysql_api:start(),
	TotalCount = 
		case mysql_api:select(<<"select count(`user_id`) from `game_spring_doll`;">>) of
	...
	case TotalCount > 0 of
		?true ->
			OkCount = 
				case mysql_api:select(<<"select `user_id`, `record` from `game_spring_doll`;">>) of
					{?ok, [?undefined]} -> 
						?MSG_SYS("undefined<-------------------", []),
						?ok;
					{?ok, DataList} ->
						TupleSize = erlang:tuple_size(#spring_doll{}),
						Fun = fun([UserId, BinRd], OldCount) ->
									  try
										  ...
										  mysql_api:update(<<"update `game_spring_doll` set `record` = ", NewBinRd/binary,
			...
			case (TotalCount - OkCount) =:= 0 of
				?true ->
					?MSG_SYS("ok");
				?false ->
					?MSG_SYS("table `game_spring_doll` count not eq ~p/~p", [OkCount, TotalCount])
			end;
		?false ->
			?MSG_SYS("table `game_spring_doll` count=0")
	end.
